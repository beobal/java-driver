...
import static com.datastax.driver.core.utils.ExecutorUtils.awaitTermination;
import static com.datastax.driver.core.utils.ExecutorUtils.shutdownNow;
import static com.datastax.driver.core.utils.FileUtils.prettyPrintMemory;
...
//    private final static BufferPoolMetrics metrics = new BufferPoolMetrics();
    ...
    @VisibleForTesting
    public static long MEMORY_USAGE_THRESHOLD = 512L * 1024L * 1024L;

    @VisibleForTesting
    public static boolean ALLOCATE_ON_HEAP_WHEN_EXAHUSTED = true;
    ...
    static final class GlobalPool implements Supplier<Chunk>, Recycler
    {
        ...
        private Chunk allocateMoreChunks()
        {
            while (true)
            {
                ...
                {
                    logger.info("Maximum memory usage reached ({}), cannot allocate chunk of {}",
                                prettyPrintMemory(MEMORY_USAGE_THRESHOLD),
            ...
            {
                logger.error("Buffer pool failed to allocate chunk of {}, current size {} ({}). " +
